...
Results of the entire b-rep generation procedure are shown in \figref{shabakaseq} for an \textit{ex vivo} human heart example \cite{cvgg}. The optimal parameters identified for robust surface generation are shown in \tabref{Mod5}. The decimation and smoothing parameters were exported to \textit{ACVD} and \textit{Meshlab}, respectively. The algorithm was rigorously tested with these parameters to produce closed, manifold, water-tight surfaces for several example inputs, as is shown in the next section.
