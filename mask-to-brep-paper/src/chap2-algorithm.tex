...
\begin{linenomath}\begin{figure}[ht!]
	...
	\subfigure[Approximating surface $\mathcal{S}$]{%
		\includegraphics[scale=0.72]{media/imgs-073022/om/dem4-32.pdf}
...
\begin{linenomath}\begin{figure}[h]
	\centering
		\includegraphics[scale=0.3]{media/om/window2.pdf}
...
\end{align}\end{linenomath}
where $\beta_i$ are adjustable weights in the range $(0,1)$ and $\sum \limits_i\beta_i = 1$.  The error $\mc{F}$ is an explicit function of the parameters $d$, $\psi$, and $\theta$, where $\psi$ and $\theta$ are the \textcolor{blue}{polar} angles of the plane normal relative to the voxel axes $(x, y, z)$, and $d$ is the plane's offset from the centroid of the window.

We seek the solution to the following unconstrained minimization problem: $\displaystyle \min_{d, \psi, \theta} \mathcal{F}(d,\psi,\theta)$, which can be solved by a variety of well-established techniques once the moments of $\Omega$ and $\Omega_p$ are defined. The moments of $\Omega_p$ are computed based on a straightforward use of voxel dimensions and the parallel axis theorem. The moments of $\Omega$, on the other hand, are first computed as a function of $d$ in the primed coordinate system, and then transformed to the $(x, y, z)$ frame based on $\psi$ and $\theta$. The relevant geometric quantities in the primed system are computed as follows \textcolor{blue}{($p, V^*, I^*_{x'x'},$ and $I^*_{y'y'}$ are sub-expressions that are used in the subsequent primed geometric quantities)}:
\begin{linenomath}\begin{align}
{p^2} &= R^2 - d^2, \\
...
I_{z'z'} &= I_{y'y'}.
\end{align}\end{linenomath}
\textcolor{blue}{The above results represent analytic evaluations of the integrals shown in (1-4), but relative to the primed coordinate system, such that $x'$ is the polar axis of the lens-shaped region $\Omega$.  These tedious but straightforward calculations are most easily carried out by first integrating over circular slabs, and lastly over the interval $x' \in [d, R]$.  In the $d < 0$ case, additional terms occur in the zeroth- and second-order moments; hence the resort to the starred sub-expressions above to achieve the most compact representation possible.}  As expected, some moments exhibit symmetries relative to the $y'$ and $z'$ axes.
