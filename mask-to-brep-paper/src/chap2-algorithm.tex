...
A \textit{segmented image} is a 3D structured array of voxels, each of which has been assigned an identifier
...
\end{itemize}
\vspace{3mm}
The basic sequence is shown (in 2D for clarity) in panels (a-c) of~\figref{vor} and panels (a-b) of ~\figref{d2dvor}. Each step in the generation of the oriented point cloud is described in turn below.
...
\subsubsection{Boundary Approximation}

Given a window $\mathcal{W}$, we seek to approximate the boundary between voxels belonging to $\mathcal{M}$ and voxels belonging to $\mathcal{W} \setminus \mathcal{M}$ by a surface $\mathcal{S}$. Assuming $\mathcal{W}$ is marked as intersecting the body boundary, an approximating sphere $\mathcal{D}$ of radius $R$ is defined, such that the center and volume of this sphere equal those of the window $\mathcal{W}$.
...
The open surface $\mathcal{S}$ that approximates the boundary takes an assumed form - for these purposes the boundary is assumed a plane. If the center of the window is defined as the origin, the planar boundary takes the form $x' = d$, where $(x',y',z')$ are the coordinates of a transformed coordinate system and $d$ is the perpendicular distance from the origin to the plane~(see~\figref{quad}).
